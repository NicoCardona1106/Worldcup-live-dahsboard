  ...
  useEffect(() => {
    const onScroll = () => {
      if (videoRef.current) {
        ...
        videoRef.current.style.transform = `translateY(${offset}px) scale(1.1)`;
      }
    };
  ...
  return (
    <section id="hero" className="relative min-h-screen flex flex-col justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-10 pt-10 w-full">
        ...
        {featured && (
          <Reveal delay={120} className="mt-10 sm:mt-14 liquid-glass max-w-3xl px-6 sm:px-10 py-8 hover:scale-[1.01] transition-transform duration-500">
              ...
              <div>
                <p className="font-anton text-5xl sm:text-7xl tracking-tighter tabular-nums">
                  {featuredIsLive || featured.status === "FINAL" ? (
                    ...
                    <span className="text-3xl sm:text-5xl text-cream/50">{featured.time}</span>
                  )}
                </p>
      ...
      <div className="relative z-10 mt-10 sm:mt-14 pb-10">
        ...
        <div className="flex gap-4 overflow-x-auto scroll-hide px-6 sm:px-10 pb-2 snap-x snap-mandatory">
          {matches.map((m) => (
            <div
              key={m.id}
              className={`liquid-glass min-w-[230px] sm:min-w-[260px] px-5 py-5 flex-shrink-0 snap-start transition-transform duration-300 hover:-translate-y-1.5 ${
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                {m.status === "LIVE" ? (
                  ...
                  <span className="font-mono text-[10px] tracking-widest text-cream/50 shrink-0">{m.time}</span>
                )}
              </div>
              <div className="flex items-center justify-between">
                ...
                <div className="font-anton text-2xl text-center w-1/3 tabular-nums">
                  {m.status === "LIVE" || m.status === "FINAL" ? (
                    ...
                    <span className="text-cream/40 text-base">{m.time}</span>
                  )}
                </div>
                <div className="flex flex-col items-center gap-1 w-1/3">
